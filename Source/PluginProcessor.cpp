...
{
    bitcrusher.prepare(sampleRate, samplesPerBlock, getTotalNumOutputChannels());
}
...
void BitcrusherAudioProcessor::processBlock (juce::AudioBuffer<float>& buffer, juce::MidiBuffer& midiMessages)
{
    ...
    auto** wp = buffer.getArrayOfWritePointers();
    
    bitcrusher.process(wp);
}

void BitcrusherAudioProcessor::updateAll()
{
    ChainSettings settings = getChainSettings(apvts);
    
    bitcrusher.setParams(settings.bitDepth, settings.downsampleRate, settings.drive, settings.clipMode);
}
